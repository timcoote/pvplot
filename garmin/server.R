...
# remove rows with missing values (na.omit)
    t1 = reactive ({ mutate (na.omit (vals()), ts = as.POSIXct (date/1000, origin= "1970-1-1")) })
    ...
    heartplot =  layer_points(ggvis (thisdata, x = ~ts), y = ~heartrate, size =0.1 ) %>% add_axis ("y", orient="left", title="Heart rate") %>%
      ...
      layer_points (prop ('y', ~speed, scale='spd'), stroke :="red", fill := "red", size=0.1) # %>% scale_numeric ("x", domain = c(min(ts()), max(ts())))
